...
def get_cache_info(data_loader, _id, fold):
    ...
    cache_gt_file = Path(
        f"work/cache/embs-{_id}-{data_loader.dataset.split}-{fold}-gt.npy"
    )
...
def _one_linear_eval(
    ...
    logging.info(
        f'🚀 Started {"Linear" if hidden_sizes == () else f"MLP with {hidden_sizes}"} evaluation:'
    )

    clf = TorchMLPClassifier2(
        hidden_layer_sizes=hidden_sizes,
        max_iter=epochs,
        learning_rate_init=lr,
        early_stopping=early_stop_epochs > 0,
        n_iter_no_change=early_stop_epochs,
        standard_scaler=standard_scaler,
        mixup=mixup,
        debug=debug,
    )
...
def make_cfg(
    config_file, task, options, extras={}, cancel_aug=False, abs_unit_sec=None
):
    cfg = load_yaml_config(config_file)
    cfg = complete_cfg(cfg, options, no_id=True)
    task_metadata, task_data, n_folds, unit_sec, weighted, balanced = get_defs(
        cfg, task
    )
...
def lineareval_downstream(
    ...
    cfg, n_folds, _, _ = make_cfg(
        config_file, task, options, extras={}, abs_unit_sec=unit_sec
    )
    ...
    logpath = app_setup_logger(
        cfg, level=logging.DEBUG
    )  # Add this when debugging deeper: level=logging.DEBUG

    scores, ar, emb_ar = [], None, None
    for fold in range(1, n_folds + 1):
        # Dataloaders.
        train_loader, valid_loader, test_loader, multi_label = create_dataloader(
            cfg, fold=fold, seed=seed, balanced_random=False, pin_memory=False
        )
        logging.info(
            f"Train:{len(train_loader.dataset)}, valid:{len(valid_loader.dataset)}, test:{len(test_loader.dataset)}, multi label:{multi_label}"
        )
        ...
        X, y = to_embeddings(
            emb_ar, train_loader, device, cfg.id, fold=fold, cache=two_pass
        )
        X_val, y_val = to_embeddings(
            emb_ar, valid_loader, device, cfg.id, fold=fold, cache=two_pass
        )
        X_test, y_test = to_embeddings(
            emb_ar, test_loader, device, cfg.id, fold=fold, cache=two_pass
        )

        if step == "2pass_1_precompute_only":
            continue

        score, df = _one_linear_eval(
            X,
            y,
            X_val,
            y_val,
    ...
    score_file = (
        logpath
        / f'{cfg.id[:-9].replace("AR_", "").replace("_", "-")}-LE_{re_hashed}_{mean_score:.5f}.csv'
    )
